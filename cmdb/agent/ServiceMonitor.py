...
from cmdb.models import ac_base, ac_cpu, ac_mem, ac_disk, ac_net
...
class Mornitor_Protocol(basic.LineReceiver):
    ...
    def update(self,line):
        ...
        基础信息入库,插入数据ac_base表
        '''
        if models.ac_base.objects.filter(hostname=agent_date['hostname'], ip=agent_date['ip']):
            base = ac_base.objects.get(hostname=agent_date['hostname'], ip=agent_date['ip'])
            base.ip = agent_date['ip']
            base.capturetime = agent_date['capturetime']
            base.cpu = agent_date['cpus']
            base.men = agent_date['mem']['mem_total']
            base.swap = agent_date['mem']['swap_total']
            base.osname = agent_date['platform']['osname']
            base.kernel = agent_date['platform']['kernel']
            base.uptime = agent_date['uptime']
            base.save()
        else:
            base = ac_base()
            base.hostname = agent_date['hostname']
            base.ip = agent_date['ip']
            base.capturetime = agent_date['capturetime']
            base.cpu = agent_date['cpus']
            base.men = agent_date['mem']['mem_total']
